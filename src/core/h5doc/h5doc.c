...
unsigned H5DOC_increaseChildnum(struct H5DOC_Stack *stack, unsigned tok_size,
				H5DOC_Token *toks)
{
    if (stack->stackptr != SIZE_MAX) {
...
unsigned H5DOC_parse(const char *input, unsigned tok_size, H5DOC_Token *toks)
{
	...
	default: /*Any other character, at the start of the token it means its a
		...
		if (currindent >
		    previndent) { /*If indentation is strictly greater, push
				     self as next child*/
		    H5DOC_increaseChildnum(&stack, tok_size, toks);
		    H5DOC_push(&stack, j);
		} else if (currindent < previndent) {
		...
		H5DOC_increaseChildnum(&stack, tok_size,
				       toks); /*Increase key parent count*/
