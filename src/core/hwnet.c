...
int cli_parse(char packet[], hwpack_client *return_pack) {
  ...
  int return_val = jsmn_parse(&parser, packet, strlen(packet), tokens, 47);
  ...
  for (gi = 0; gi < return_val; gi++) {
    ...
    if (tokens[i].type == JSMN_PRIMITIVE) {
      fprintf(stderr,
              "At Token %i [%i,%i]: PRIMITIVE type not allowed by "
    ...
    /*
    printf("Token %i START-%i SIZE-%i END-%i TYPE-%s VALUE:\n%s\n\n", i + 1,
           tokens[i].start, tokens[i].size, tokens[i].end, jsmn_typestr[i],
           packetsub[i]);
    */
  }
	int hi;
  for (hi = 0; hi < return_val; hi++) {
    if (strcmp(packetsub[hi], "user") == 0 && tokens[hi].type == JSMN_STRING &&
        tokens[hi].size == 1) {

      if (strcmp(packetsub[hi + 2], "id") == 0 &&
          tokens[hi].type == JSMN_STRING && tokens[hi + 3].size <= 4) {
				unsigned int d;
        for (d = 0; d <= tokens[hi + 3].size; d++) {
          if (strtol(packetsub[hi + 4 + d], NULL, 10) != 0)
            idlist[d] = strtol(packetsub[hi + 4 + d], NULL, 10);
          //printf("IDs: %u\n", idlist[d]);
        }
      }
      ...
      for ( f = 0; f < tokens[hi + 1].size; f++) {
        if (strtol(packetsub[hi + 2 + (f * 7)], NULL, 10) != 0 &&
            strtol(packetsub[hi + 2 + (f * 7)], NULL, 10) == idlist[f]) {

          composed_packet.player_data[idlist[f]].thruster_2 =
              strtol(packetsub[hi + 6 + (f * 7)], NULL, 10);
          composed_packet.player_data[idlist[f]].thruster_1 =
              strtol(packetsub[hi + 7 + (f * 7)], NULL, 10);
          composed_packet.player_data[idlist[f]].steer =
              strtol(packetsub[hi + 8 + (f * 7)], NULL, 10);

        } else if (strcmp(packetsub[hi + 2 + (f * 7)], "exten") == 0 &&
                   tokens[hi].type == JSMN_STRING) {
          strcpy(composed_packet.extension, packetsub[hi + 4 + (f * 7)]);
        }
      }
...
int serv_parse(char packet[], hwpack_server *return_pack) {
  ...
  int return_val = jsmn_parse(&parser, packet, strlen(packet), tokens, 70);
  if (return_val < 0) {
    fprintf(stderr, "Invalid JSON %i\n", return_val);
    *return_pack = error_packet;
    return return_val;
  ...
  for (li = 0; li < return_val; li++) {
    if (strcmp(packetsub[li], "data") == 0 && tokens[li].type == JSMN_STRING &&
        tokens[li].size == 1) {
			unsigned int f;
      for (f = 0; f < tokens[li + 1].size; f++) {
        if (strtol(packetsub[li + 2 + (f * 7)], NULL, 10) != 0) {

          composed_packet
              .player_data[strtol(packetsub[li + 2 + (f * 7)], NULL, 10)]
              .position1 = strtol(packetsub[li + 4 + (f * 7)], NULL, 10);
          composed_packet
              .player_data[strtol(packetsub[li + 2 + (f * 7)], NULL, 10)]
              .position2 = strtol(packetsub[li + 5 + (f * 7)], NULL, 10);
          switch (packetsub[li + 6 + (f * 7)][0]) {
          case 'o':
            composed_packet
                .player_data[strtol(packetsub[li + 2 + (f * 7)], NULL, 10)]
                .flags_1 = o;
