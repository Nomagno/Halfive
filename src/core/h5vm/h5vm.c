...
*/
#define ISADR(typeinfo, operand_position) (((typeinfo | (1 << (2 - operand_position))) != typeinfo))
#define ISLIT(typeinfo, operand_position)\
	(((typeinfo | (1 << (2 - operand_position))) == typeinfo) && ((typeinfo | 8) != typeinfo))
...
#define _RETURN rwinf->was_err = !!return_code; return return_code;
...
#define GETVAR(out, operands, operand_position, rvar) {                                                        \
	...
	} else if (ISADR(operands[2], operand_position) && DATA[operands[operand_position - 1]] != NULL) {         \
	...
	} else if (ISPTR(operands[2], operand_position) && PTR_VALID(DATA, operands[operand_position - 1])) {      \
...
#define GETTYPE(operands, operand_position)                                                      \
	((ISPTR(operands[2], operand_position)) ? ptr : ((ISADR(operands[2], operand_position)) ? adr : lit))
...
unsigned H5VM_Execute(H5VM_GeneralMemory *program, H5VM_ReadWriteInfo *rwinf) {
	*rwinf = (H5VM_ReadWriteInfo){0};
	...
	h5uint subval = 0; /*For 'func' instruction. This can't be a safe 'ret' place, so it's an error code*/
	...
	_set:
		...
		if (!donot_save) { /*We don't care about any memory
			...
			if (dest_type == lit) {
				return_code = 3; /*ERROR: Wrong usage! Can't set a literal, that's nonsensical*/
				_RETURN;
