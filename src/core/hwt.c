...
hwtrack get_track(FILE *track_file) {
  ...
  hwtrack track;
	...
	/*FILL ERROR STRUCT*/
  hwtrack error; strcpy(error.name, "ERROR"); 
  error.biome.the_biome = forest; error.biome.main_color = green;
	error.time = day;
	error.max_players = 1;
  ...
  toml_table_t *track_toml = toml_parse_file(track_file, toml_errbuf, 100);
  if (!track_toml) {
  ...
  int a_point;
    ...
    local_params[i] = toml_array_in(segments_subtable[i], "local_param");
    if (local_params[i]) {
      local_params_values[i] = toml_string_at(local_params[i], 0);
    }
    if ((local_params[i]) &&
    ...
    for (e = 0; e < 6; e++) {
      ...
      if (!points[i][e].ok) {
      	fprintf(stderr, "At segment %d, point %d", i+1, e+1);
        perror("Segment %d, point %d: Not Found. Abort\n");
				return error;
      }
    ...
    if ((local_params[i]) &&
      ...
      track.segments[i].racing_line = finish_line;
               ...
               (strcmp(local_params_values[i].u.s, "checkpoint") == 0)) {
  ...
  if (finish_line == 0) {
    fprintf(stderr, "No finish_line detected in the local parameters. Please review "
           "them.\n");
		return error;
  }
