...
/*Halfive Physics component (OUTDATED)*/
...
/*
Old notes:
[h5vi graphics + audio] <- [Monolithic Object Scheduler (H5PHY MOS, handles
physics)]
...
typedef struct {
    ...
    char info[16]; /*String, ASCII printable characters, following format:
		     ...
		     Characters 10-13: Decimal number string, position in
		     category Characters 14-16: Decimal number string, driver
		     number*/
    ...
    h5uchar magnetic_adapt;   /*0 TO 255,
				  The capability of the magnets to compensate for
				 excessive aero   grip and other situations where
				 the floor would scrape the floor*/
...
FUNKY RESULTS, HOWEVER THERE IS NOTHING INHERENTLY WRONG WITH IT)*/
    ...
    H5VM_GeneralMemory
	computer; /*More or less 140KBs of storage with default settings*/
    h5uchar status_lights; /*16 lights,  each two contiguous bits is a light,
			      4-colour. Mapped 0x1010 to 0x1013*/
    h5uchar status_screen[16]; /*8x8 screen, each two contiguous bits are
				  pixels, 4-colour. Mapped 0x1014 to 0x1023*/
#endif
	...
	rideheight; /*Ride height, in mm, lower means more stable and faster,
		       but might scrape the floor. Influenced by magnetic
		       adaptability*/
    h5uchar prev_roll,
	roll; /*Roll relative to floor, in degrees. Depending on ride height,
		 car width, length and height,  might scrape the floor*/
    h5uchar prev_pitch,
	pitch; /*Pitch relative to floor, in degrees. Depending on ride height,
		  car width, length and height, might scrape the floor*/
    h5uchar prev_yaw, yaw; /*Absolute yaw, biggest determinant of car direction.
			      A fast change in yaw might make the car spin*/
    ...
    h5ulong states[2];
    h5uchar engine1[4]; /*Store how ramped up it is, its current power,
			   degradation info, and flags*/
    h5uchar engine2[4]; /*Store how ramped up it is, its current power,
			   degradation info, and flags*/
...
extern unsigned H5_sim(H5_World *stage,
		       h5uint delta /*Time since last frame, in milliseconds*/);
extern unsigned
H5_transformServer(const H5_World *stage, H5NET_ServerPacket *serv,
		   enum H5NET_ModeEnum mode); /*Convert H5_World to a packet
						 ready to send to clients*/
extern unsigned H5_transformClient(
    H5_World *stage,
    const H5NET_ClientPacket
	*cli); /*Mix client packet information with current H5_World*/
#endif
